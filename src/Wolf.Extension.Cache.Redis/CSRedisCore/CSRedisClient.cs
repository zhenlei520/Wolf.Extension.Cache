...
namespace CSRedis
{
    public partial class CSRedisClient : IDisposable
    {
        ...
        /// 按 key 规则分区存储
        /// </summary>
        public ConcurrentDictionary<string, RedisClientPool> Nodes { get; } =
            new ConcurrentDictionary<string, RedisClientPool>();
            ...
            if (type.IsValueType)
            {
                ...
                switch (basename)
                {
                    ...
                    case "System.DateTime":
                        return ((DateTime) value).ToString("yyyy-MM-ddTHH:mm:sszzzz",
        ...
        internal (T1, T2)[] DeserializeRedisValueTuple1Internal<T1, T2>(Tuple<byte[], T2>[] value)
        {
            ...
            for (var a = 0; a < value.Length; a++)
                list[a] = (this.DeserializeRedisValueInternal<T1>(value[a].Item1), value[a].Item2);
            return list;
        }
        ...
        internal (T2, T1)[] DeserializeRedisValueTuple2Internal<T2, T1>(Tuple<T2, byte[]>[] value)
        {
            ...
            for (var a = 0; a < value.Length; a++)
                list[a] = (value[a].Item1, this.DeserializeRedisValueInternal<T1>(value[a].Item2));
            return list;
        }

        internal Dictionary<TKey, TValue> DeserializeRedisValueDictionaryInternal<TKey, TValue>(
            Dictionary<TKey, byte[]> value)
        {
            if (value == null) return null;
        ...
        public CSRedisClient(IJsonFactory jsonFactory, string connectionString) : this(jsonFactory, null, new string[0],
            false, null, connectionString)
        {
        }
        ...
        public CSRedisClient(IJsonFactory jsonFactory, string connectionString, string[] sentinels,
            bool readOnly = false) : this(jsonFactory, null, sentinels, readOnly, null, connectionString)
        {
        }
        ...
        public CSRedisClient(IJsonFactory jsonFactory, string connectionString, string[] sentinels, bool readOnly,
            SentinelMasterConverter convert) : this(jsonFactory, null, sentinels, readOnly, convert, connectionString)
        {
        }
        ...
        public CSRedisClient(IJsonFactory jsonFactory, Func<string, string> NodeRule, params string[] connectionStrings)
            : this(jsonFactory, NodeRule, null, false, null, connectionStrings)
        {
        }
            ...
            bool readOnly, SentinelMasterConverter convert = null, params string[] connectionStrings)
        {
            ...
            this.NodeRuleRaw = key =>
            {
                ...
                var slot = GetClusterSlot(string.Concat(prefix,
                    key)); //redis-cluster 模式，选取第一个 connectionString prefix 前辍求 slot
                if (SlotCache.TryGetValue(slot, out var slotIndex) &&
                    NodesIndex.TryGetValue(slotIndex, out var slotKey))
                {
            ...
            if (ing)
            {
                ...
                new Thread(() =>
                {
                    while (true)
                    {
                        ...
                        {
                            SentinelMasterValue =
                                SentinelManager.Connect(SentinelMasterName, pool._policy._connectTimeout);
        ...
        RedisClientPool GetRedirectPool((bool isMoved, bool isAsk, ushort slot, string endpoint) redirect,
        ...
        T NodesNotSupport<T>(string[] keys, T defaultValue, Func<Object<RedisClient>, string[], T> callback)
        {
            ...
            var rules = Nodes.Count > 1
                ? keys.Select(a => NodeRuleRaw(a)).Distinct()
                : new[] {Nodes.FirstOrDefault().Key};
            ...
            Func<string[], (string, T)[]> getData)
        {
            ...
            if (fieldsMGet.Any())
            {
                ...
                foreach (var d in data)
                {
                    ...
                    mset[msetIndex++] =
                        this.SerializeObject((d.Item2, (long) DateTime.Now.Subtract(_dt1970).TotalSeconds));
                ...
                foreach (var fieldNull in fieldsMGet.Keys)
                {
                    ...
                    mset[msetIndex++] =
                        this.SerializeObject((default(T), (long) DateTime.Now.Subtract(_dt1970).TotalSeconds));
                }
        ...
        public partial class NodesServerManagerProvider
        {
            ...
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] BgRewriteAof() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.BgRewriteAof()))).ToArray();

            /// <summary>
            /// 在后台异步保存当前数据库的数据到磁盘
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] BgSave() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.BgSave()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, string value)[] ClientKill(string ip, int port) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ClientKill(ip, port)))).ToArray();
            ...
            /// <returns></returns>
            public (string node, long value)[] ClientKill(string addr = null, string id = null,
                ClientKillType? type = null, bool? skipMe = null) => _csredis.Nodes.Values.Select(a =>
                    _csredis.GetAndExecute(a, c => (a.Key, c.Value.ClientKill(addr, id, type?.ToString(), skipMe))))
                .ToArray();
            ...
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] ClientList() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ClientList()))).ToArray();

            /// <summary>
            /// 获取连接的名称
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] ClientGetName() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ClientGetName()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, string value)[] ClientPause(TimeSpan timeout) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ClientPause(timeout)))).ToArray();
            ...
            /// <returns></returns>
            public (string node, string value)[] ClientSetName(string connectionName) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ClientSetName(connectionName)))).ToArray();

            /// <summary>
            /// 返回当前服务器时间
            /// </summary>
            /// <returns></returns>
            public (string node, DateTime value)[] Time() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.Time()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, Dictionary<string, string> value)[] ConfigGet(string parameter) => _csredis.Nodes
                .Values.Select(a => _csredis.GetAndExecute(a,
                    c => (a.Key, c.Value.ConfigGet(parameter).ToDictionary(z => z.Item1, y => y.Item2)))).ToArray();

            /// <summary>
            /// 对启动 Redis 服务器时所指定的 redis.conf 配置文件进行改写
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] ConfigRewrite() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ConfigRewrite()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, string value)[] ConfigSet(string parameter, string value) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ConfigSet(parameter, value)))).ToArray();

            /// <summary>
            /// 重置 INFO 命令中的某些统计数据
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] ConfigResetStat() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.ConfigResetStat()))).ToArray();

            /// <summary>
            /// 返回当前数据库的 key 的数量
            /// </summary>
            /// <returns></returns>
            public (string node, long value)[] DbSize() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.DbSize()))).ToArray();

            /// <summary>
            /// 让 Redis 服务崩溃
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] DebugSegFault() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.DebugSegFault()))).ToArray();

            /// <summary>
            /// 删除所有数据库的所有key
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] FlushAll() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.FlushAll()))).ToArray();

            /// <summary>
            /// 删除当前数据库的所有key
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] FlushDb() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.FlushDb()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, string value)[] Info(InfoSection? section = null) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.Info(section?.ToString())))).ToArray();

            /// <summary>
            /// 返回最近一次 Redis 成功将数据保存到磁盘上的时间
            /// </summary>
            /// <returns></returns>
            public (string node, DateTime value)[] LastSave() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.LastSave()))).ToArray();
            ...
            /// </summary>
            /// <returns></returns>
            public (string node, RedisRole value)[] Role() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.Role()))).ToArray();

            /// <summary>
            /// 同步保存数据到硬盘
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] Save() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.Save()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, string value)[] Shutdown(bool isSave = true) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.Shutdown(isSave)))).ToArray();
            ...
            /// <param name="port">端口</param>
            /// <returns></returns>
            public (string node, string value)[] SlaveOf(string host, int port) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.SlaveOf(host, port)))).ToArray();

            /// <summary>
            /// 从属服务器执行命令 SLAVEOF NO ONE 将使得这个从属服务器关闭复制功能，并从从属服务器转变回主服务器，原来同步所得的数据集不会被丢弃。
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] SlaveOfNoOne() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.SlaveOfNoOne()))).ToArray();
            ...
            /// <returns></returns>
            public (string node, RedisSlowLogEntry[] value)[] SlowLogGet(long? count = null) => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.SlowLogGet(count)))).ToArray();

            /// <summary>
            /// 管理 redis 的慢日志，总数量
            /// </summary>
            /// <returns></returns>
            public (string node, long value)[] SlowLogLen() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.SlowLogLen()))).ToArray();

            /// <summary>
            /// 管理 redis 的慢日志，清空
            /// </summary>
            /// <returns></returns>
            public (string node, string value)[] SlowLogReset() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.SlowLogReset()))).ToArray();

            /// <summary>
            /// 用于复制功能(replication)的内部命令
            /// </summary>
            /// <returns></returns>
            public (string node, byte[] value)[] Sync() => _csredis.Nodes.Values
                .Select(a => _csredis.GetAndExecute(a, c => (a.Key, c.Value.Sync()))).ToArray();
        }
        ...
        public partial class NodeServerManagerProvider
        {
            ...
            public string ClientKill(string ip, int port) =>
                _csredis.GetAndExecute(_pool, c => c.Value.ClientKill(ip, port));
            ...
            /// <param name="skipMe">跳过自己</param>
            /// <returns></returns>
            public long ClientKill(string addr = null, string id = null, ClientKillType? type = null,
                bool? skipMe = null) =>
            ...
            public string ClientPause(TimeSpan timeout) =>
                _csredis.GetAndExecute(_pool, c => c.Value.ClientPause(timeout));
            ...
            /// <param name="parameter">参数</param>
            /// <returns></returns>
            public Dictionary<string, string> ConfigGet(string parameter) => _csredis
                .GetAndExecute(_pool, c => c.Value.ConfigGet(parameter)).ToDictionary(z => z.Item1, y => y.Item2);
            ...
            /// <returns></returns>
            public string Info(InfoSection? section = null) =>
                _csredis.GetAndExecute(_pool, c => c.Value.Info(section?.ToString()));
            ...
            public string SlaveOf(string host, int port) =>
                _csredis.GetAndExecute(_pool, c => c.Value.SlaveOf(host, port));
        ...
        public bool Ping(string nodeKey) =>
            GetAndExecute(GetNodeOrThrowNotFound(nodeKey), c => c.Value.Ping()) == "PONG";
        ...
        private bool Quit(string nodeKey) =>
            GetAndExecute(GetNodeOrThrowNotFound(nodeKey), c => c.Value.Quit()) == "OK";
        ...
        /// <param name="sha1">脚本缓存的sha1</param>
        /// <returns></returns>
        public bool[] ScriptExists(params string[] sha1) => Nodes.Select(a =>
            GetAndExecute(a.Value, c => c.Value.ScriptExists(sha1))?.Where(z => z == false).Any() == false).ToArray();
        ...
        /// <param name="script">Lua 脚本</param>
        /// <returns></returns>
        public string ScriptLoad(string script) => Nodes.Select(a =>
            GetAndExecute(a.Value, c => (c.Pool.Policy.Name.ToString(), c.Value.ScriptLoad(script)))).First().Item2;
        ...
        public Dictionary<string, long> PubSubNumSub(params string[] channels) => ExecuteArray(channels, (c, k) =>
        {
            ...
            return c.Value.PubSubNumSub(k.Select(z =>
                    string.IsNullOrEmpty(prefix) == false && z.StartsWith(prefix) ? z.Substring(prefix.Length) : z)
                .ToArray());
        ...
        public SubscribeObject Subscribe(params (string, Action<SubscribeMessageEventArgs>)[] channels)
        {
            ...
            List<(string[] keys, Object<RedisClient> conn)> subscrs =
                new List<(string[] keys, Object<RedisClient> conn)>();
            foreach (var r in rules)
            {
        ...
        public class SubscribeObject : IDisposable
        {
                ...
                Dictionary<string, Action<SubscribeMessageEventArgs>> onMessageDic)
            {
                ...
                AppDomain.CurrentDomain.ProcessExit += (s1, e1) => { this.Dispose(); };
                try
                {
            ...
            private void Subscribe(object state)
            {
                ...
                var testCSRedis_Subscribe_Keepalive =
                ...
                EventHandler<RedisSubscriptionReceivedEventArgs> SubscriptionReceived = (a, b) =>
                {
                    try
                    {
                        if (b.Message.Type == "message" && this.OnMessageDic != null &&
                            this.OnMessageDic.TryGetValue(b.Message.Channel, out var action) == true)
                        {
                    ...
                    try
                    {
                        foreach (var chan in subscr.chans)
                        {
                            ...
                            if (testKeepalived == false)
                            {
                                ...
                                try
                                {
                                    subscr.conn.Value.Socket?.Shutdown(System.Net.Sockets.SocketShutdown.Both);
                                }
                                catch
                                {
                                }
            ...
            public void Dispose()
            {
                ...
                if (this.Subscrs != null)
                {
                    foreach (var subscr in this.Subscrs)
                    {
                        ...
                        try
                        {
                            subscr.conn.Value.ReceiveTimeout =
                                (subscr.conn.Pool as RedisClientPool)._policy._syncTimeout;
                        }
                        catch
                        {
                        }
...
return 0", $"CSRedisPSubscribe{psubscribeKey}", "", trylong.ToString());
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Console.ForegroundColor = ConsoleColor.White;
                        Console.Write(
                            $"模糊订阅出错【{pool.Key}】(channels:{string.Join(",", Channels)})：{ex.Message}\r\n{ex.StackTrace}");
                        Console.BackgroundColor = bgcolor;
                        Console.ForegroundColor = forecolor;
                        Console.WriteLine();
                    }
                ...
                while (true)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Console.ForegroundColor = ConsoleColor.White;
                        Console.Write(
                            $"模糊订阅出错【{pool.Key}】(channels:{string.Join(",", Channels)})：{ex.Message}，3秒后重连。。。");
                        Console.BackgroundColor = bgcolor;
                        Console.ForegroundColor = forecolor;
                        Console.WriteLine();
                        Thread.CurrentThread.Join(1000 * 3);
        ...
        /// <returns></returns>
        public SubscribeListBroadcastObject SubscribeListBroadcast(string listKey, string clientId,
            Action<string> onMessage)
        {
            this.HSetNx($"{listKey}_SubscribeListBroadcast", clientId, 1);
            var subobj = new SubscribeListBroadcastObject
            {
                OnDispose = () => { this.HDel($"{listKey}_SubscribeListBroadcast", clientId); }
            };
            //订阅其他端转发的消息
        ...
        /// <returns></returns>
        public bool PfMerge(string destKey, params string[] sourceKeys) => NodesNotSupport(
            new[] {destKey}.Concat(sourceKeys).ToArray(), false,
        ...
        public (T member, decimal score)[] ZPopMax<T>(string key, long count) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(ExecuteScalar(key,
                (c, k) => c.Value.ZPopMaxBytes(k, count)));
        ...
        public (T member, decimal score)[] ZPopMin<T>(string key, long count) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(ExecuteScalar(key,
                (c, k) => c.Value.ZPopMinBytes(k, count)));
        ...
        public long ZAdd(string key, params (decimal, object)[] scoreMembers)
        {
            ...
            var args = scoreMembers
                .Select(a => new Tuple<decimal, object>(a.Item1, this.SerializeRedisValueInternal(a.Item2))).ToArray();
        ...
        public long ZCount(string key, string min, string max) =>
            ExecuteScalar(key, (c, k) => c.Value.ZCount(k, min, max));
        ...
        public string[] ZRange(string key, long start, long stop) =>
            ExecuteScalar(key, (c, k) => c.Value.ZRange(k, start, stop, false));
        ...
        public T[] ZRange<T>(string key, long start, long stop) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key,
                (c, k) => c.Value.ZRangeBytes(k, start, stop, false)));
        ...
        public (string member, decimal score)[] ZRangeWithScores(string key, long start, long stop) =>
            ExecuteScalar(key, (c, k) => c.Value.ZRangeWithScores(k, start, stop)).Select(a => (a.Item1, a.Item2))
                .ToArray();
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (string member, decimal score)[] ZRangeByScoreWithScores(string key, decimal min, decimal max,
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (T member, decimal score)[] ZRangeByScoreWithScores<T>(string key, decimal min, decimal max,
            long? count = null, long offset = 0) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(ExecuteScalar(key,
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (string member, decimal score)[] ZRangeByScoreWithScores(string key, string min, string max,
            long? count = null, long offset = 0) =>
            ExecuteScalar(key, (c, k) => c.Value.ZRangeByScoreWithScores(k, min, max, offset, count))
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (T member, decimal score)[] ZRangeByScoreWithScores<T>(string key, string min, string max,
            long? count = null, long offset = 0) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(ExecuteScalar(key,
        ...
        /// <param name="max">分数最大值 decimal.MaxValue 10</param>
        /// <returns></returns>
        public long ZRemRangeByScore(string key, decimal min, decimal max) => ExecuteScalar(key,
        ...
        public string[] ZRevRange(string key, long start, long stop) =>
            ExecuteScalar(key, (c, k) => c.Value.ZRevRange(k, start, stop, false));
        ...
        public T[] ZRevRange<T>(string key, long start, long stop) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key,
                (c, k) => c.Value.ZRevRangeBytes(k, start, stop, false)));
        ...
        public (string member, decimal score)[] ZRevRangeWithScores(string key, long start, long stop) =>
            ExecuteScalar(key, (c, k) => c.Value.ZRevRangeWithScores(k, start, stop)).Select(a => (a.Item1, a.Item2))
                .ToArray();
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (string member, decimal score)[] ZRevRangeByScoreWithScores(string key, decimal max, decimal min,
            long? count = null, long offset = 0) =>
            ExecuteScalar(key,
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (T member, decimal score)[] ZRevRangeByScoreWithScores<T>(string key, decimal max, decimal min,
            long? count = null, long offset = 0) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(ExecuteScalar(key,
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (string member, decimal score)[] ZRevRangeByScoreWithScores(string key, string max, string min,
            long? count = null, long offset = 0) =>
        ...
        /// <param name="offset">返回条件偏移位置</param>
        /// <returns></returns>
        public (T member, decimal score)[] ZRevRangeByScoreWithScores<T>(string key, string max, string min,
            long? count = null, long offset = 0) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(ExecuteScalar(key,
        ...
        /// <param name="count">数量</param>
        /// <returns></returns>
        public RedisScan<(string member, decimal score)> ZScan(string key, long cursor, string pattern = null,
            long? count = null)
        {
        ...
        /// <param name="count">数量</param>
        /// <returns></returns>
        public RedisScan<(T member, decimal score)> ZScan<T>(string key, long cursor, string pattern = null,
            long? count = null)
        {
            var scan = ExecuteScalar(key, (c, k) => c.Value.ZScanBytes(k, cursor, pattern, count));
            return new RedisScan<(T, decimal)>(scan.Cursor,
                this.DeserializeRedisValueTuple1Internal<T, decimal>(scan.Items));
        }
        ...
        public T[] ZRangeByLex<T>(string key, string min, string max, long? count = null, long offset = 0) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key,
                (c, k) => c.Value.ZRangeBytesByLex(k, min, max, offset, count)));
        ...
        public T[] SDiff<T>(params string[] keys) =>
            this.DeserializeRedisValueArrayInternal<T>(NodesNotSupport(keys, new byte[0][],
                (c, k) => c.Value.SDiffBytes(k)));
        ...
        /// <returns></returns>
        public long SDiffStore(string destination, params string[] keys) => NodesNotSupport(
            new[] {destination}.Concat(keys).ToArray(), 0,
        ...
        public string[] SInter(params string[] keys) =>
            NodesNotSupport(keys, new string[0], (c, k) => c.Value.SInter(k));
        ...
        /// <param name="keys">一个或多个无序集合，不含prefix前辍</param>
        /// <returns></returns>
        public long SInterStore(string destination, params string[] keys) => NodesNotSupport(
            new[] {destination}.Concat(keys).ToArray(), 0,
        ...
        public T[] SMembers<T>(string key) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key, (c, k) => c.Value.SMembersBytes(k)));
        ...
        public T SPop<T>(string key) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.SPopBytes(k)));
        ...
        public T[] SPop<T>(string key, long count) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key, (c, k) => c.Value.SPopBytes(k, count)));
        ...
        public T SRandMember<T>(string key) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.SRandMemberBytes(k)));
        ...
        public string[] SRandMembers(string key, int count = 1) =>
            ExecuteScalar(key, (c, k) => c.Value.SRandMembers(k, count));
        ...
        public T[] SRandMembers<T>(string key, int count = 1) =>
            this.DeserializeRedisValueArrayInternal<T>(
                ExecuteScalar(key, (c, k) => c.Value.SRandMembersBytes(k, count)));
        ...
        public string[] SUnion(params string[] keys) =>
            NodesNotSupport(keys, new string[0], (c, k) => c.Value.SUnion(k));
        ...
        /// <param name="keys">一个或多个无序集合，不含prefix前辍</param>
        /// <returns></returns>
        public long SUnionStore(string destination, params string[] keys) => NodesNotSupport(
            new[] {destination}.Concat(keys).ToArray(), 0,
        ...
        public (string key, T value)? BLPopWithKey<T>(int timeout, params string[] keys)
        {
            ...
            if (tuple == null) return null;
            return (rkeys?.Where(b => b == tuple.Item1).First() ?? tuple.Item1,
                this.DeserializeRedisValueInternal<T>(tuple.Item2));
        }
        ...
        public string BLPop(int timeout, params string[] keys) =>
            NodesNotSupport(keys, null, (c, k) => c.Value.BLPop(timeout, k));
        ...
        public T BLPop<T>(int timeout, params string[] keys) =>
            this.DeserializeRedisValueInternal<T>(NodesNotSupport(keys, null,
                (c, k) => c.Value.BLPopBytes(timeout, k)));
        ...
        public string BRPop(int timeout, params string[] keys) =>
            NodesNotSupport(keys, null, (c, k) => c.Value.BRPop(timeout, k));
        ...
        public T BRPop<T>(int timeout, params string[] keys) =>
            this.DeserializeRedisValueInternal<T>(NodesNotSupport(keys, null,
                (c, k) => c.Value.BRPopBytes(timeout, k)));
        ...
        /// <param name="timeout">超时(秒)</param>
        /// <returns></returns>
        public string BRPopLPush(string source, string destination, int timeout) => NodesNotSupport(
            new[] {source, destination}, null, (c, k) => c.Value.BRPopLPush(k.First(), k.Last(), timeout));
        ...
        /// <param name="timeout">超时(秒)</param>
        /// <returns></returns>
        public T BRPopLPush<T>(string source, string destination, int timeout) => this.DeserializeRedisValueInternal<T>(
            NodesNotSupport(new[] {source, destination}, null,
        ...
        public T LIndex<T>(string key, long index) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.LIndexBytes(k, index)));
        ...
        public T LPop<T>(string key) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.LPopBytes(k)));
        ...
        public string[] LRange(string key, long start, long stop) =>
            ExecuteScalar(key, (c, k) => c.Value.LRange(k, start, stop));
        ...
        public T[] LRange<T>(string key, long start, long stop) =>
            this.DeserializeRedisValueArrayInternal<T>(
                ExecuteScalar(key, (c, k) => c.Value.LRangeBytes(k, start, stop)));
        ...
        public bool LTrim(string key, long start, long stop) =>
            ExecuteScalar(key, (c, k) => c.Value.LTrim(k, start, stop)) == "OK";
        ...
        public T RPop<T>(string key) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.RPopBytes(k)));
        ...
        /// <param name="destination">目标key，不含prefix前辍</param>
        /// <returns></returns>
        public string RPopLPush(string source, string destination) => NodesNotSupport(new[] {source, destination}, null,
            (c, k) => c.Value.RPopLPush(k.First(), k.Last()));
        ...
        /// <param name="destination">目标key，不含prefix前辍</param>
        /// <returns></returns>
        public T RPopLPush<T>(string source, string destination) => this.DeserializeRedisValueInternal<T>(
            NodesNotSupport(new[] {source, destination}, null, (c, k) => c.Value.RPopBytesLPush(k.First(), k.Last())));
        ...
        /// <param name="fields">字段</param>
        /// <returns></returns>
        public long HDel(string key, params string[] fields) => fields == null || fields.Any() == false
            ? 0
            : ExecuteScalar(key, (c, k) => c.Value.HDel(k, fields));
        ...
        public T HGet<T>(string key, string field) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.HGetBytes(k, field)));
        ...
        public Dictionary<string, T> HGetAll<T>(string key) =>
            this.DeserializeRedisValueDictionaryInternal<string, T>(ExecuteScalar(key,
                (c, k) => c.Value.HGetAllBytes(k)));
        ...
        /// <param name="fields">字段</param>
        /// <returns></returns>
        public string[] HMGet(string key, params string[] fields) => fields == null || fields.Any() == false
            ? new string[0]
            : ExecuteScalar(key, (c, k) => c.Value.HMGet(k, fields));
        ...
        /// <returns></returns>
        public T[] HMGet<T>(string key, params string[] fields) => fields == null || fields.Any() == false
            ? new T[0]
            : this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key, (c, k) => c.Value.HMGetBytes(k, fields)));
        ...
        public T[] HVals<T>(string key) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key, (c, k) => c.Value.HValsBytes(k)));
        ...
        /// <param name="count">数量</param>
        /// <returns></returns>
        public RedisScan<(string field, string value)> HScan(string key, long cursor, string pattern = null,
            long? count = null)
        {
        ...
        /// <param name="count">数量</param>
        /// <returns></returns>
        public RedisScan<(string field, T value)> HScan<T>(string key, long cursor, string pattern = null,
            long? count = null)
        {
            var scan = ExecuteScalar(key, (c, k) => c.Value.HScanBytes(k, cursor, pattern, count));
            return new RedisScan<(string, T)>(scan.Cursor,
                scan.Items.Select(z => (z.Item1, this.DeserializeRedisValueInternal<T>(z.Item2))).ToArray());
        }
        ...
        public long BitCount(string key, long start, long end) =>
            ExecuteScalar(key, (c, k) => c.Value.BitCount(k, start, end));
        ...
        public T Get<T>(string key) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.GetBytes(k)));
        ...
        public string GetRange(string key, long start, long end) =>
            ExecuteScalar(key, (c, k) => c.Value.GetRange(k, start, end));
        ...
        public T GetRange<T>(string key, long start, long end) =>
            this.DeserializeRedisValueInternal<T>(ExecuteScalar(key, (c, k) => c.Value.GetRangeBytes(k, start, end)));
        ...
        public decimal IncrByFloat(string key, decimal value) =>
            ExecuteScalar(key, (c, k) => c.Value.IncrByFloat(k, value));
        ...
        public T[] MGet<T>(params string[] keys) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteArray(keys, (c, k) => c.Value.MGetBytes(k)));
        ...
        public bool Set(string key, object value, int expireSeconds = -1, RedisExistence? exists = null)
        {
            ...
            if (expireSeconds > 0 && exists == null)
                return ExecuteScalar(key, (c, k) => c.Value.Set(k, redisValule, expireSeconds, null)) == "OK";
            if (expireSeconds > 0 && exists != null)
        ...
        public bool Set(string key, object value, TimeSpan expire, RedisExistence? exists = null)
        {
            ...
            if (expire <= TimeSpan.Zero && exists != null)
                return ExecuteScalar(key, (c, k) => c.Value.Set(k, redisValule, null, exists)) == "OK";
            if (expire > TimeSpan.Zero && exists == null)
                return ExecuteScalar(key, (c, k) => c.Value.Set(k, redisValule, expire, null)) == "OK";
            if (expire > TimeSpan.Zero && exists != null)
        ...
        public bool SetBit(string key, uint offset, bool value) =>
            ExecuteScalar(key, (c, k) => c.Value.SetBit(k, offset, value));
        ...
        public long? ObjectRefCount(string key) =>
            ExecuteScalar(key, (c, k) => c.Value.Object(RedisObjectSubCommand.RefCount, k));
        ...
        public long? ObjectIdleTime(string key) =>
            ExecuteScalar(key, (c, k) => c.Value.Object(RedisObjectSubCommand.IdleTime, k));
        ...
        public bool PExpire(string key, int milliseconds) =>
            ExecuteScalar(key, (c, k) => c.Value.PExpire(k, milliseconds));
        ...
        public bool PExpireAt(string key, DateTime expire) =>
            ExecuteScalar(key, (c, k) => c.Value.PExpireAt(k, expire));
        ...
        /// <param name="newKey">新名称，不含prefix前辍</param>
        /// <returns></returns>
        public bool RenameNx(string key, string newKey) => NodesNotSupport(new[] {key, newKey}, false,
            (c, k) => c.Value.RenameNx(k.First(), k.Last()));
        ...
        /// <returns></returns>
        public string[] Sort(string key, long? count = null, long offset = 0, string by = null,
        ...
        public long SortAndStore(string key, string destination, long? count = null, long offset = 0, string by = null,
            RedisSortDir? dir = null, bool? isAlpha = null, params string[] get) =>
            NodesNotSupport(key,
                (c, k) => c.Value.SortAndStore(k, (c.Pool as RedisClientPool)?.Prefix + destination, offset, count, by,
                    dir, isAlpha, get));
        ...
        public KeyType Type(string key) =>
            Enum.TryParse(ExecuteScalar(key, (c, k) => c.Value.Type(k)), true, out KeyType tryenum)
                ? tryenum
                : KeyType.None;
        ...
        public long GeoAdd(string key, params (decimal longitude, decimal latitude, object member)[] values)
        {
            ...
            var args = values.Select(z => (z.longitude, z.latitude, this.SerializeRedisValueInternal(z.member)))
                .ToArray();
        ...
        /// <returns></returns>
        public string[] GeoRadius(string key, decimal longitude, decimal latitude, decimal radius,
            GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadius(k, longitude, latitude, radius, unit, count, sorting, false, false,
                        false))
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public T[] GeoRadius<T>(string key, decimal longitude, decimal latitude, decimal radius,
            GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                (c, k) => c.Value.GeoRadiusBytes(k, longitude, latitude, radius, unit, count, sorting, false, false,
                    false)).Select(a => this.DeserializeRedisValueInternal<T>(a.member)).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public (string member, decimal dist)[] GeoRadiusWithDist(string key, decimal longitude, decimal latitude,
            decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadius(k, longitude, latitude, radius, unit, count, sorting, false, true,
                        false))
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public (T member, decimal dist)[] GeoRadiusWithDist<T>(string key, decimal longitude, decimal latitude,
            decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                (c, k) => c.Value.GeoRadiusBytes(k, longitude, latitude, radius, unit, count, sorting, false, true,
                    false)).Select(a => (this.DeserializeRedisValueInternal<T>(a.member), a.dist)).ToArray();
        ...
        private (string member, decimal longitude, decimal latitude)[] GeoRadiusWithCoord(string key, decimal longitude,
            decimal latitude, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
            GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadius(k, longitude, latitude, radius, unit, count, sorting, true, false,
                        false))
        ...
        private (T member, decimal longitude, decimal latitude)[] GeoRadiusWithCoord<T>(string key, decimal longitude,
            ...
            GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusBytes(k, longitude, latitude, radius, unit, count, sorting, true, false,
                        false)).Select(a => (this.DeserializeRedisValueInternal<T>(a.member), a.longitude, a.latitude))
                .ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public (string member, decimal dist, decimal longitude, decimal latitude)[] GeoRadiusWithDistAndCoord(
            string key, decimal longitude, decimal latitude, decimal radius, GeoUnit unit = GeoUnit.m,
            long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadius(k, longitude, latitude, radius, unit, count, sorting, true, true,
                        false))
        ...
        public (T member, decimal dist, decimal longitude, decimal latitude)[] GeoRadiusWithDistAndCoord<T>(string key,
            ...
            GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                (c, k) => c.Value.GeoRadiusBytes(k, longitude, latitude, radius, unit, count, sorting, true, true,
                    false)).Select(a =>
                (this.DeserializeRedisValueInternal<T>(a.member), a.dist, a.longitude, a.latitude)).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public string[] GeoRadiusByMember(string key, object member, decimal radius, GeoUnit unit = GeoUnit.m,
            long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusByMember(k, member, radius, unit, count, sorting, false, false, false))
                .Select(a => a.member).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public T[] GeoRadiusByMember<T>(string key, object member, decimal radius, GeoUnit unit = GeoUnit.m,
            long? count = null, GeoOrderBy? sorting = null) =>
            this.DeserializeRedisValueArrayInternal<T>(ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusBytesByMember(k, member, radius, unit, count, sorting, false, false,
                        false))
                .Select(a => a.member).ToArray());
        ...
        public (string member, decimal dist)[] GeoRadiusByMemberWithDist(string key, object member, decimal radius,
            GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusByMember(k, member, radius, unit, count, sorting, false, true, false))
                .Select(a => (a.member, a.dist)).ToArray();
        ...
        public (T member, decimal dist)[] GeoRadiusByMemberWithDist<T>(string key, object member, decimal radius,
            GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusBytesByMember(k, member, radius, unit, count, sorting, false, true,
                        false))
                .Select(a => (this.DeserializeRedisValueInternal<T>(a.member), a.dist)).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        private (string member, decimal longitude, decimal latitude)[] GeoRadiusByMemberWithCoord(string key,
            object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusByMember(k, member, radius, unit, count, sorting, true, false, false))
                .Select(a => (a.member, a.longitude, a.latitude)).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        private (T member, decimal longitude, decimal latitude)[] GeoRadiusByMemberWithCoord<T>(string key,
            object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusBytesByMember(k, member, radius, unit, count, sorting, true, false,
                        false))
                .Select(a => (this.DeserializeRedisValueInternal<T>(a.member), a.longitude, a.latitude)).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public (string member, decimal dist, decimal longitude, decimal latitude)[] GeoRadiusByMemberWithDistAndCoord(
            string key, object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
            GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusByMember(k, member, radius, unit, count, sorting, true, true, false))
                .Select(a => (a.member, a.dist, a.longitude, a.latitude)).ToArray();
        ...
        /// <param name="sorting">排序</param>
        /// <returns></returns>
        public (T member, decimal dist, decimal longitude, decimal latitude)[] GeoRadiusByMemberWithDistAndCoord<T>(
            string key, object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
            GeoOrderBy? sorting = null) =>
            ExecuteScalar(key,
                    (c, k) => c.Value.GeoRadiusBytesByMember(k, member, radius, unit, count, sorting, true, true,
                        false))
                .Select(a => (this.DeserializeRedisValueInternal<T>(a.member), a.dist, a.longitude, a.latitude))
                .ToArray();
        ...
        public long XAck(string key, string group, string id) =>
            ExecuteScalar(key, (c, k) => c.Value.XAck(k, group, id));
        ...
        /// <param name="fieldValues">键值对数组</param>
        /// <returns></returns>
        public string XAdd(string key, string id = "*", params (string, string)[] fieldValues) =>
            XAdd(key, 0, id, fieldValues);
        ...
        /// <param name="id">消息id</param>
        /// <returns></returns>
        public (string id, string[] items)[] XClaim(string key, string group, string consumer, long minIdleTime,
            params string[] id) =>
        ...
        /// <returns></returns>
        public (string id, string[] items)[] XClaim(string key, string group, string consumer, long minIdleTime,
            string[] id, long idle, long retryCount, bool force) =>
        ...
        public string[] XClaimJustId(string key, string group, string consumer, long minIdleTime, string[] id,
            ...
            ExecuteScalar(key,
                (c, k) => c.Value.XClaimJustId(k, group, consumer, minIdleTime, id, idle, retryCount, force));
        ...
        public bool XGroupDestroy(string key, string group) =>
            ExecuteScalar(key, (c, k) => c.Value.XGroupDestroy(k, group));
        ...
        /// <param name="key">不含prefix前辍</param>
        /// <returns></returns>
        public (long length, long radixTreeKeys, long radixTreeNodes, long groups, string lastGeneratedId, (string id,
            string[] items) firstEntry, (string id, string[] items) lastEntry) XInfoStream(string key) =>
        ...
        /// <param name="group"></param>
        /// <returns></returns>
        public (long count, string minId, string maxId, (string consumer, long count)[] pendings) XPending(string key,
            string group) =>
        ...
        /// <returns></returns>
        public (string id, string consumer, long idle, long transferTimes)[] XPending(string key, string group,
            string start, string end, long count, string consumer = null) =>
        ...
        /// <param name="streams">(key,id) 数组</param>
        /// <returns></returns>
        public (string key, (string id, string[] items)[] data)[] XReadGroup(string group, string consumer, long count,
            long block, params (string key, string id)[] streams) =>
            NodesNotSupport(streams.Select(a => a.key).ToArray(), null,
                (c, k) => c.Value.XReadGroup(group, consumer, count, block,
                    streams.Select((a, i) => (k[i], a.id)).ToArray()));
        ...
        public bool BfReserve(string key, decimal errorRate, long capacity, int expansion = 2,
            bool nonScaling = false) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.BfReserve(k, errorRate, capacity, expansion, nonScaling))) ==
            "OK";
        ...
        public bool[] BfMAdd(string key, object[] items) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.BfMAdd(k,
                    items.Select(item => this.SerializeRedisValueInternal(item)).ToArray())));

        public bool[] BfInsert(string key, object[] items, long? capacity = null, string error = null,
            int expansion = 2, bool noCreate = false, bool nonScaling = false) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.BfInsert(k,
                    items.Select(item => this.SerializeRedisValueInternal(item)).ToArray(), capacity, error, expansion,
                    noCreate, nonScaling)));

        public bool BfExists(string key, object item) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.BfExists(k, this.SerializeRedisValueInternal(item))));
        ...
        public (long capacity, long size, long numberOfFilters, long numberOfItemsInserted, long expansionRate)
            BfInfo(string key) =>
        ...
        public bool CfReserve(string key, long capacity, long? bucketSize = null, long? maxIterations = null,
            int? expansion = null) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CfReserve(k, capacity, bucketSize, maxIterations, expansion))) ==
            "OK";

        public bool CfAdd(string key, object item) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CfAdd(false, k, this.SerializeRedisValueInternal(item))));

        public bool CfAddNx(string key, object item) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CfAdd(true, k, this.SerializeRedisValueInternal(item))));
        ...
        public bool[] CfInsert(string key, object[] items, long? capacity = null, bool noCreate = false) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CfInsert(false, k,
                    items.Select(item => this.SerializeRedisValueInternal(item)).ToArray(), capacity, noCreate)));
        ...
        public bool CfExists(string key, object item) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CfExists(k, this.SerializeRedisValueInternal(item))));
        ...
        public long CfCount(string key, object item) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CfCount(k, this.SerializeRedisValueInternal(item))));
        ...
        public (long size, long numberOfBuckets, long numberOfFilter, long numberOfItemsInserted, long
            numberOfItemsDeleted, long bucketSize, long expansionRate, long maxIteration) CfInfo(string key) =>
        ...
        public long[] CmsIncrBy(string key, params (object item, long increment)[] items) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.CmsIncrBy(k,
                    items.Select(item => (this.SerializeRedisValueInternal(item.item), item.increment)).ToArray())));
        ...
        public bool CmsMerge(string dest, long numKeys, string[] src, long[] weights) =>
            NodesNotSupport(new[] {dest}.Concat(src).ToArray(), null,
                (c, k) => c.Value.Write(RedisCommands.CmsMerge(k.FirstOrDefault(), numKeys,
                    k.Where((_, idx) => idx > 0).ToArray(), weights))) == "OK";
        ...
        public bool TopkReserve(string key, long topk, long width, long depth, decimal decay) =>
            ExecuteScalar(key, (c, k) => c.Value.Write(RedisCommands.TopkReserve(k, topk, width, depth, decay))) ==
            "OK";

        public string[] TopkAdd(string key, object[] items) =>
            ExecuteScalar(key,
                (c, k) => c.Value.Write(RedisCommands.TopkAdd(k,
                    items.Select(item => this.SerializeRedisValueInternal(item)).ToArray())));
    ...
    public class CSRedisClientLock : IDisposable
    {
        ...
        internal CSRedisClientLock(CSRedisClient rds, string name, string value, int timeoutSeconds,
            double refreshSeconds, bool autoDelay)
        {
            _client = rds;
            _name = name;
            _value = value;
